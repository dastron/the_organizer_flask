        ...
        for row in spamreader:
            ...
            try:
                ...
                group_id = db.session.query(Group).filter(
                    Group.attribute == str(row[1])).one()
                group_id = group_id.id
                # print group_id;
            except:
                # Create Group
                group_id = Group.add(None, row[1], '0', True).insert()
            #title, headline, description, primary_key, url, amazon_url,thumbnail
            item_id = Item.add(
                'a9cb6bcf-7d53-4e04-8482-c40c980794b0',
                row[0],
                row[2],
                '',
                row[3],
            ...
            GroupMap.add(item_id, group_id, row[1], '0', True).insert()

            for image in images[row[3]]:
                ItemImage.add(item_id, '/static/' + image).insert()

            short_id = AttributeMap.getID('short_id')
            ItemAttribute.add(item_id, short_id, 'short_id',
                              row[3], True).insert()

            barcode_id = AttributeMap.getID('barcode')
            ItemAttribute.add(item_id, barcode_id,
                              'barcode', row[4], True).insert()
