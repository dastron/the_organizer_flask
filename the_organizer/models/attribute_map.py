...
class AttributeMap(AnATTR, db.Model):
    ...
    @staticmethod
    def add(attribute, value, active):
        return AttributeMap(attribute=attribute, value=value, active=active)
    ...
    def getID(attribute):
        try:
            # Get Item ID
            returnID = db.session.query(AttributeMap).filter(
                AttributeMap.attribute == attribute).one()

            return returnID.id
        except:
            # Create Item
            return AttributeMap.add(attribute, '0', True).insert()
