...
class Part(AnObject, db.Model):
    ...
    images = db.relationship("PartImage")
    ...
    def add(title, headline, description, primary_key, url, amazon_url,thumbnail):
        return Part(title=title, headline=headline, description=description, primary_key=primary_key, url=url, amazon_url=amazon_url, thumbnail=thumbnail)
